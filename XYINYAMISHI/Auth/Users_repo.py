...
class UsersRepository:
    ...
    def delete_user(self, id:str):
        self.cur.execute(
            "DELETE FROM registered_users WHERE id = %s", (id,))
        self.conn.commit()
    ...
    def get_user_hash(self, id: str):
        self.cur.execute(
            "SELECT password FROM registered_users WHERE id = %s", (id,))
        result = self.cur.fetchone()
        if not result:
            return None
...
class DriversRepository(UsersRepository):
    ...
    def delete_driver(self, id: str):
        self.cur.execute(
            "DELETE FROM registered_drivers WHERE id = %s", (id,))
        self.conn.commit()
